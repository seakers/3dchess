...
def main(
    ...
    # save events
    events_path = os.path.join('./events', f'events_seed-{seed}.csv')
...
def create_events(experiments_dir : str,
    ...
    event_list = [(event.event_type, event.severity, event.location[0], event.location[1], event.t_start, event.t_end, event.t_corr, event.id) 
                for event in events]

    # compile list of events
    return pd.DataFrame(data=event_list, 
                        columns=['event_type','severity','lat [deg]','lon [deg]','start time [s]','end time [s]','decorrelation time [s]', 'id'])
...
if __name__ == "__main__":
    ...
    grids_names = [
        "HydroLAKES_polys_v10_simple_10000",
        # "HydroRIVERS_v10_simple_10000",
    ]
