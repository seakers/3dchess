...
def main(
    ...
    # set scenario name
    parent_scenario_name = 'preplanner_study'
    ...
    grid_file = os.path.join(scenario_dir, 'resources', 'grids', f'hydrolakes_dataset.csv')
    grid : pd.DataFrame = pd.read_csv(grid_file)
    points_considered = [
                            int(np.floor(i * len(grid)))
                            for i in [1/10, 1/4, 1/3, 1/2, 1.0 ]
                        ]   

    # count number of runs to be made
    n_runs : int = min(len(experiments_df), upper_bound-lower_bound) \
                    * len(preplanners) * len(points_considered) \
                        if not debug else len(preplanners) * len(points_considered)
    ...
    with tqdm.tqdm(total=n_runs) as pbar:
        ...
        for _,row in experiments_df.iterrows():           
            ...
            for preplanner in preplanners:
                for n_points in points_considered:
