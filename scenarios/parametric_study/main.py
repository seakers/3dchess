                        ...
                        for bundle_size in bundle_sizes:
                            ...
                            if not (same_events and pregenerated_events):
                                if event_clustering == 'uniform':
                                    grid_path = create_uniform_grid(scenario_dir, n_events)
                                elif event_clustering == 'clustered':
                                    grid_path = create_clustered_grid(scenario_dir, n_events)
                                else:
                                    raise NotImplementedError(f'event clustering of type {event_clustering} not supported.')

                                create_events(scenario_dir, grid_path, sim_duration, n_events, event_duration, min_severity, max_severity, measurement_list)
                                pregenerated_events = True
                                
                            scenario_specs['grid'] = [{
                                                        "@type": "customGrid",
                                                        "covGridFilePath": os.path.join(scenario_dir, 'resources', f'random_grid.csv')
                                                    }]
                            scenario_specs['scenario']['events'] = {
                                                        "@type": "PREDEF", 
                                                        "eventsPath" : os.path.join(scenario_dir, 'resources', f'random_events.csv')
                                                    }
                            ...
                            scenario_name = f'{parent_scenario_name}_{i}_{preplanner}-{period}-{horizon}_{replanner}'
...
def clear_orbitdata(scenario_dir : str) -> None:
    ...
    if os.path.exists(orbitdata_path):       
        ...
        shutil.rmtree(orbitdata_path)

def create_uniform_grid(scenario_dir, n_events : int, lat_spacing : float = 0.1, lon_spacing : float = 0.1) -> str:
    ...
    # groundpoints = [(np.round(lat,1),np.round(lon,1))
    #                 for lat,lon in groundpoints]
    groundpoints.sort()
    ...
    # save to csv
    grid_path : str = os.path.join(scenario_dir, 'resources', f'random_grid.csv')
    df.to_csv(grid_path,index=False)

    # return address
    return grid_path

def create_clustered_grid(scenario_dir, n_events : int, variance : float = 1.0, n_clusters : float = 100) -> str:
...
def create_events(scenario_dir : str, grid_path : str, sim_duration : float, n_events : int, event_duration : float, min_severity : float, max_severity : float, measurements : list) -> str:
    # load coverage grid
    ...
    assert all([(lat,lon) in grid.values for lat,lon,*_ in events])
    ...
    events_path = os.path.join(scenario_dir, 'resources', 'random_events.csv')
