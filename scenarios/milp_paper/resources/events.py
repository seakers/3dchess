...
def main(experiments_name : str,
    ...
    n_runs : int = len(experiments_df) / (len(experiments_df['Planner'].unique()) * len(experiments_df['Model'].unique()))
    print(F'NUMBER OF EVENTS TO GENERATE: ~{n_runs}')

    # set simulation duration in hours
    sim_duration : float = 24.0 / 24.0

    experiments_df.pop('Planner')
    experiments_df.pop('Model')
...
if __name__ == "__main__":
    ...
    parser.add_argument('-e',
                        '--experiment-name', 
                        help='name of experiment being used to select the location of events',
                        type=str,
                        required=False,
                        default='milp_lakes_testcase_nov_2025_seed-1000')
