...
class AbstractPreplanner(ABC):
    ...
    def _schedule_broadcasts(self, 
        ...
        By default it only schedules the relay of any incoming relay messages
        """
        # initialize list of broadcasts to be done
        broadcasts = []       

        if len(orbitdata) <= 1:
            # if there are no other agents in the simulation, do not transmit anything
            return broadcasts
        ...
        # if there are messages to broadcast, find best path for each message
        if requests_to_broadcast or self.pending_relays:
        ...
        for req in requests_to_broadcast:        
            ...
            if t_start <= state.t + self.period:
                ...
                self.pending_relays.append(msg)

        # schedule message relay
        for relay_msg in self.pending_relays:
            relay_msg : SimulationMessage

            assert relay_msg.path

            # find next destination and access time
            if relay_msg.path:
                next_dst = relay_msg.path[0]
                ...
                raise NotImplementedError('Need to implement message relay protocol.')

            if t_start < np.Inf:
                # if found, create broadcast action
                broadcast_action = BroadcastMessageAction(relay_msg.to_dict(), t_start)
    ...
    def _create_broadcast_path(self, agent_name : str, orbitdata : dict, t : float) -> tuple:
        ...
        access_all = True
        for access_i in [orbitdata[agent_name].get_next_agent_access(agent, t) for agent in agents]:
            access_i : TimeInterval

            for access_j in [orbitdata[agent_name].get_next_agent_access(agent, t) for agent in agents]:
                access_j : TimeInterval
...
class FIFOPreplanner(AbstractPreplanner):
    ...
    def _schedule_broadcasts(self, 
                             ...
                             orbitdata: dict) -> list:
        """
        Schedules an announcement any generated request,
        an annoucement of the current plan,
        the completion of each measurement after it's been performed
        """
        broadcasts : list = super()._schedule_broadcasts(state, measurements, orbitdata)

        # # schedule performed measurement broadcasts
        if self.collaboration and len(orbitdata) > 1:

            # find best path for broadcasting data
            path, t_start = self._create_broadcast_path(state.agent_name, orbitdata, state.t)
            ...
            if measurements:
                ...
                msg = PlanMessage(state.agent_name, 
                                state.agent_name, 
                                [measurement_action.to_dict() for measurement_action in measurements],
                                state.t,
                                path=path)
                broadcast_action = BroadcastMessageAction(msg.to_dict(), t_start)
