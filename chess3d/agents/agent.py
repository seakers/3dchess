...
class AbstractAgent(Agent):
    ...
    async def update_state_environment(self, env_resp : BusMessage)-> list:
        # initiate list of senses
        senses = []
        
        # convert bus to list of messages
        env_resp_msgs = [message_from_dict(**msg) for msg in env_resp.msgs]

        # separate messages by type
        env_state_msgs = [msg for msg in env_resp_msgs if isinstance(msg, AgentStateMessage)]
        env_conn_msgs = [msg for msg in env_resp_msgs if isinstance(msg, AgentConnectivityUpdate)]

        # process state update messages
        for state_msg in env_state_msgs:
            # update state
            state_msg = AgentStateMessage(  self.get_element_name(), 
                                            self.get_element_name(),
                                            self.state.to_dict()
        ...
        env_conn_msgs.sort(key=lambda x: x.connected, reverse=True) # process connect messages first
        for conn_msg in env_conn_msgs:
            if bool(conn_msg.connected):                                        
                # subscribe from broadcasts
                self.subscribe_to_broadcasts(conn_msg.target)
    ...
    async def __empty_queue(self, q : asyncio.Queue) -> list:
        ...
        while not q.empty():
            ...
            await asyncio.sleep(1e-2 * np.random.random())
        return msgs
    ...
    async def perform_broadcast(self, action : BroadcastMessageAction) -> str:        
        ...
        # ask for a small time wait to allow other agents to process the broadcast
    ...
    async def perform_wait(self, action : WaitAction, save : bool = True) -> str:
        ...
        completed = t_new > action.t_end or abs(t_new - action.t_end) < 1e-6

        # return action completion
        return AgentAction.COMPLETED if completed else AgentAction.ABORTED
    ...
    async def sim_wait(self, delay: float, timeout : float=np.Inf) -> None:
        try:  
            ...
            # cancel tic request if confirmation was not received when this task was interupted
    ...
    def print_results(self):
        try:
            ...
            for routine in self.stats:
                ...
                routine_dir = os.path.join(f"{self.results_path}/runtime", f"time_series-planner_{routine}.parquet")
            ...
            if isinstance(self.preplanner, AbstractPeriodicPlanner):
                for routine in self.preplanner.stats:
                    ...
                    routine_df = pd.DataFrame(data=time_series, columns=['dt'])
                    routine_dir = os.path.join(f"{self.results_path}/runtime", f"time_series-preplanner_{routine}.parquet")
                    routine_df.to_parquet(routine_dir,index=False)
            ...
            if isinstance(self.replanner, AbstractReactivePlanner):
                for routine in self.replanner.stats:
                    ...
                    routine_df = pd.DataFrame(data=time_series, columns=['dt'])
                    routine_dir = os.path.join(f"{self.results_path}/runtime", f"time_series-replanner_{routine}.parquet")
                    routine_df.to_parquet(routine_dir,index=False)

            stats_df = pd.DataFrame(data, columns=headers)
            stats_df.to_parquet(f"{self.results_path}/planner_runtime_stats.parquet", index=False)
